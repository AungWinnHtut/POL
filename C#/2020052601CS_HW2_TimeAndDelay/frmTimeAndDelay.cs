...
namespace _2020052501CS_HW1_TimeAndDelay
{
    public partial class frmTimeAndDelay : Form
    {
            ...
            else
            {
                if ((bAM == true) && (rdoAM.Checked == true) || (bAM == false) && (rdoPM.Checked == true))
                {
                    if (iEndTimeH == iAnsH && iEndTimeM == iAnsM)
                    {
                        ...
                        funSendMsg("Right Answer", "အေျဖမွန္ပါတယ္။ ဂုဏ္ယူပါတယ္။");
                        funGenQ();
                    }
                    else
                    {
        ...
        public void funSendMsg(string msg)
        {
            frmMsgBox frmMsg = new frmMsgBox(msg);
            frmMsg.ShowDialog();
        }
        public void funSendMsg(string title, string msg)
        {
            frmMsgBox frmMsg = new frmMsgBox(title, msg);
            frmMsg.ShowDialog();
        }
        ...
        public void funGenQ()
        {
            ...
            iStartTimeH = rand.Next(1, 13);
            iStartTimeM = rand.Next(1, 60);

            iDelayH = rand.Next(1, 13);
            iDelayM = rand.Next(1, 60);
